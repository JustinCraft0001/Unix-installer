...
if [ ! -f "artisan" ]; then
    ...
    mkdir -p tempdown && cd tempdown && git clone https://github.com/JustinCraft0001/Unix-installer .
    cp -r installer/unix/app ..
    cp -r installer/unix/config ..
    cp -r installer/unix/database ..
    cp -r installer/unix/public ..
    cp -r installer/unix/resources ..
    cp -r installer/unix/routes ..
